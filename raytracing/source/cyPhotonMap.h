...
namespace cy {
	...
	template <int maxPhotons>
	inline void PhotonMap::EstimateIrradiance(Color &irrad, Point3f &direction, float radius, const Point3f &pos, const Point3f *normal, float ellipticity, FilterType filterType) const
	{
		irrad.SetBlack();
		direction.Zero();
	...
	inline bool PhotonMap::GetNearestPhoton(PhotonMap::Photon &photon, float radius, const Point3f &pos, const Point3f *normal, float ellipticity) const
	{
		float found_dist2[2];
